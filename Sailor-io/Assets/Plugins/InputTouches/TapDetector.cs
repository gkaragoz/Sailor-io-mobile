...
public class TapDetector : MonoBehaviour {
	...
	IEnumerator FingerRoutine(int index){
		...
		while(true){
			...
			if(!longTap && Time.time-startTime>longTapTime && Vector2.Distance(lastPos, startPos)<maxTapDisplacementAllowance){
		...
		//check for shortTap
		if(Time.time-startTime<=shortTapTime && Vector2.Distance(lastPos, startPos)<maxTapDisplacementAllowance){
